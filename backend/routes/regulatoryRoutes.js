...
router.post('/add', authenticateToken, requireRoles(['SuperAdmin', 'Regulatory']), upload.single('documentFile'), async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ success: false, message: 'Failed to create certificate.' }); }
});
...
router.put('/:id', authenticateToken, requireRoles(['SuperAdmin', 'Regulatory']), async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ success: false, message: 'Failed to update certificate.' }); }
});
...
router.delete('/:id', authenticateToken, requireRoles(['SuperAdmin', 'Regulatory']), async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ success: false, message: 'Failed to delete certificate.' }); }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        res.json(normalized);
    } catch (err) { res.status(500).json({ success: false, message: 'Failed to fetch certificates.' }); }
});
